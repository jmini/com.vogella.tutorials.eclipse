...
|Ensure that `org.eclipse.equinox.ds` is added as a dependency in your product. Remember to press `Add Required Plug-ins` after adding a new dependency.

|"Event Admin service is not available, unable to publish event org.osgi.service.event.Event" message during start up.
|Ensure that `org.eclipse.equinox.event` is added as a dependency in your product. Remember to press `Add Required Plug-ins` after adding a new dependency.
|===
